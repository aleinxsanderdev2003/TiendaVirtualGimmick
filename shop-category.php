							...
							<div class="categorie_filter">
								<ul>
									<li>
										...
										<?php
										if(isset($_GET['pag'])){
											echo '?pag='.$_GET['pag'].'&st='.$_GET['st'].'&to=estrellas_five';
											}else{
												echo '?to=estrellas_five&st='.$_GET['st'].'';
											}
											?>
											...
											<span>(
												<?php
												$estrellas5 = mysqli_query($mysqli,"SELECT COUNT(*) AS estrellas5 FROM productos WHERE estrellas='5' and categoria=".$_GET['st']."");
									...
									<li>
										...
										<?php
										if(isset($_GET['pag'])){
											echo '?pag='.$_GET['pag'].'&st='.$_GET['st'].'&to=estrellas_four';
											}else{
												echo '?to=estrellas_four&st='.$_GET['st'].'';
											}
											?>
											...
											<span>(
												<?php
												$estrellas4 = mysqli_query($mysqli,"SELECT COUNT(*) AS estrellas4 FROM productos WHERE estrellas='4' and categoria=".$_GET['st']."");
									...
									<li>
										...
										<?php
										if(isset($_GET['pag'])){
											echo '?pag='.$_GET['pag'].'&st='.$_GET['st'].'&to=estrellas_three';
											}else{
												echo '?to=estrellas_three&st='.$_GET['st'].'';
											}
											?>
											...
											<span>(
												<?php
												$estrellas3 = mysqli_query($mysqli,"SELECT COUNT(*) AS estrellas3 FROM productos WHERE estrellas='3' and categoria=".$_GET['st']."");
									...
									<li>
										...
										<?php
										if(isset($_GET['pag'])){
											echo '?pag='.$_GET['pag'].'&st='.$_GET['st'].'&to=estrellas_two';
											}else{
												echo '?to=estrellas_two&st='.$_GET['st'].'';
											}
											?>
											...
											<span>(
												<?php
												$estrellas2 = mysqli_query($mysqli,"SELECT COUNT(*) AS estrellas2 FROM productos WHERE estrellas='2' and categoria=".$_GET['st']."");
									...
									<li>
										...
										<?php
										if(isset($_GET['pag'])){
											echo '?pag='.$_GET['pag'].'&st='.$_GET['st'].'&to=estrellas_one';
											}else{
												echo '?to=estrellas_one&st='.$_GET['st'].'';
											}
											?>
											...
											<span>(
												<?php
												$estrellas1 = mysqli_query($mysqli,"SELECT COUNT(*) AS estrellas1 FROM productos WHERE estrellas='1' and categoria=".$_GET['st']."");
							...
							<div class="categorie_filter">
								<ul>
									<li>
										...
										<?php
										if(isset($_GET['pag'])){
											echo '?pag='.$_GET['pag'].'&st='.$_GET['st'].'&to=Nuevo';
											}else{
												echo '?to=Nuevo&st='.$_GET['st'].'';
											}
											?>
											...
											<i class="fa fa-caret-right"></i> Nuevo <span>(
												<?php
												$condicion2 = mysqli_query($mysqli,"SELECT COUNT(*) AS condicion_nuevo FROM productos WHERE condicion='Nuevo' and categoria=".$_GET['st']."" );
									...
									<li>
										...
										<?php
										if(isset($_GET['pag'])){
											echo '?pag='.$_GET['pag'].'&st='.$_GET['st'].'&to=Usado';
											}else{
												echo '?to=Usado&st='.$_GET['st'].'';
											}
											?>
											...
											<i class="fa fa-caret-right"></i> Usado <span>(
												<?php
												$condicion1 = mysqli_query($mysqli,"SELECT COUNT(*) AS condicion_usado FROM productos WHERE condicion='Usado' and categoria=".$_GET['st']."");
				...
				<div class="col-lg-9 col-md-8">
					<div class="categorie_d_right">
						<div class="categorie_product_toolbar mb-30">
							...
							$productos = mysqli_query($mysqli,"SELECT COUNT(*) AS productos FROM productos WHERE estado = '0' and categoria=".$_GET['st']." ");
						...
						<div class="tab-content" id="myTabContent">
							<div class="tab-pane fade show active" id="large" role="tabpanel">
								<div class="row cate_tab_product">
									...
									$productos = "SELECT * FROM productos WHERE estado = '0' and categoria=".$_GET['st']." and ".$filtro." ORDER BY id_producto DESC LIMIT $inicial,$limite";
									...
									if ($result > 0) {
										...
										Pero tenemos otros productos que te van a encantar.
										</b>
										</h5>
										</center>
										</div>
						...
						<div class="page_numbers_toolbar">
							<ul>
								...
								{  
									...
									if(isset($_GET['to'])){
										echo '?pag='.$anterior.'&st='.$_GET['st'].'&to='.$_GET['to'].'';
									}else{
										echo '?to=products&st='.$_GET['st'].'&pag='.$anterior.'';
									}
								...
								for ($i = 0; $i<($pag + 1); $i++) { 
									...
									if($i==$pag){
										...
										if(isset($_GET['to'])){
											echo '?pag='.$i.'&st='.$_GET['st'].'&to='.$_GET['to'].'';
										}else{
											echo '?to=products&st='.$_GET['st'].'&pag='.$i.'';
										}
								...
								{  
									...
									if(isset($_GET['to'])){
										echo '?pag='.$siguiente.'&st='.$_GET['st'].'&to='.$_GET['to'].'';
									}else{
										echo '?to=products&st='.$_GET['st'].'&pag='.$siguiente.'';
									}
